<?php
/**
 * Genrolla Theme - Dummy Post Generator (Debug Version)
 * 
 * INSTRUCTIONS:
 ...
 * 3. DELETE file ini setelah selesai (security risk!)
 */
...
echo '<html><head><title>Genrolla Post Generator - Debug</title></head><body>';
echo '<h1>Genrolla Post Generator - Debug Mode</h1>';
echo '<hr>';

// Check if wp-load.php exists
if (!file_exists('wp-load.php')) {
    echo '<p style="color:red;">❌ ERROR: wp-load.php not found in current directory!</p>';
    ...
    echo '</ul>';
    echo '<p><strong>FIX:</strong> Upload this file to WordPress ROOT directory (same level as wp-config.php)</p>';
    die();
}

echo '<p>✅ wp-load.php found. Loading WordPress...</p>';
...
if (!is_user_logged_in()) {
    echo '<p style="color:red;">❌ ERROR: You must be logged in to WordPress first!</p>';
    echo '<p><a href="' . admin_url() . '">Login to WordPress</a> first, then come back to this URL.</p>';
    die();
}

if (!current_user_can('administrator')) {
    echo '<p style="color:red;">❌ ERROR: Only administrators can run this script!</p>';
    die();
}

echo '<p>✅ User is admin. Proceeding...</p><hr>';
...
echo '<h2>Creating Posts...</h2>';

$created_count = 0;
$errors = array();

foreach ($sample_posts as $index => $post_data) {
    echo '<p>Creating post #' . ($index + 1) . ': <strong>' . esc_html($post_data['title']) . '</strong>... ';
    ...
    try {
        ...
        if (is_wp_error($post_id)) {
            $errors[] = 'Post #' . ($index + 1) . ': ' . $post_id->get_error_message();
            echo '<span style="color:red;">❌ FAILED</span></p>';
        } else {
            // Add tags
            wp_set_post_tags($post_id, $post_data['tags'], true);
            echo '<span style="color:green;">✅ SUCCESS (ID: ' . $post_id . ')</span></p>';
            $created_count++;
        }
    } catch (Exception $e) {
        $errors[] = 'Post #' . ($index + 1) . ': ' . $e->getMessage();
...
echo '<hr>';
echo '<h2>Summary</h2>';
echo '<p>✅ Successfully created <strong>' . $created_count . '/5</strong> posts!</p>';
...
echo '<p>🔗 <a href="' . admin_url('edit.php') . '">View All Posts in WordPress</a></p>';
echo '<p style="background:#fff3cd; padding:15px; border:1px solid #ffc107;"><strong>⚠️ IMPORTANT:</strong> Delete this file NOW for security!<br>';
echo 'Command: <code>rm ' . __FILE__ . '</code></p>';
